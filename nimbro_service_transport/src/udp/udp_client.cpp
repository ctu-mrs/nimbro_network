...
public:
  ...
  virtual bool call(ros::ServiceCallbackHelperCallParams& params) {
    ROS_INFO("[SERVICE_CLIENT]: [%s]: Calling.", m_name.c_str());
    ...
    if (call_result) {
      ROS_INFO("[SERVICE_CLIENT]: [%s]: Call was successful (dt = %.4f sec).", m_name.c_str(), call_time.toSec());
    } else {
      ROS_ERROR("[SERVICE_CLIENT]: [%s]: Call error (dt = %.4f sec).", m_name.c_str(), call_time.toSec());
    }
    return call_result;
  }
...
UDPClient::UDPClient(const std::string& robot_hostname, const std::string& robot_addr, std::map<std::string, std::vector<std::string>>& services,
  ...
  for (auto const& serv_type : services) {
    for (const std::string& topic_name : services[serv_type.first]) {
      ROS_INFO("[SERVICE_CLIENT]: Advertising service [%s] with type [%s].", topic_name.c_str(), serv_type.first.c_str());
  ...
  if (!m_topic_prefix.empty()) {
    topic_name << "/" << m_topic_prefix << "/network/service_status";
  } else {
  ...
  ROS_WARN("[SERVICE_CLIENT]: Service client initialized.");
}
...
bool UDPClient::call(const std::string& name, ros::ServiceCallbackHelperCallParams& params) {
  ...
  for (int i = 0; i < m_call_repeats; i++) {
    if (send(m_fd, buffer.data(), buffer.size(), 0) != (int)buffer.size()) {
      ROS_ERROR("[SERVICE_CLIENT]: [%s] Could not send UDP data: %s", m_remote_hostname.c_str(), strerror(errno));
  ...
  if (!gotAck) {
    ROS_ERROR("[SERVICE_CLIENT]: [%s]: Have not received Ack in timeout!", name.c_str());
    return false;
  }
  ...
  if (gotMsg) {
    ...
    ROS_ERROR("[SERVICE_CLIENT]: [%s]: Have not received the response in timeout!", name.c_str());
...
void UDPClient::handlePacket() {
  ...
  if (bytes < 0) {
    if (errno == ECONNREFUSED) {
      ROS_ERROR("[SERVICE_CLIENT]: [%s], Got negative ICMP reply. Is the UDP server running?", m_remote_hostname.c_str());
      return;
    }
  ...
  if (bytes < (int)sizeof(ServiceCallAcknowledgement)) {
    ROS_ERROR("[SERVICE_CLIENT]: [%s] Short response packet, ignoring...", m_remote_hostname.c_str());
    return;
  }
  ...
  if (bytes == (int)sizeof(ServiceCallAcknowledgement)) {
    ...
    ROS_WARN("[SERVICE_CLIENT]: [%s] Received unexpected UDP service packet answer, ignoring", m_remote_hostname.c_str());
    return;
  }
  ...
  if (resp->response_length() + sizeof(ServiceCallResponse) > (size_t)bytes) {
    ROS_ERROR("[SERVICE_CLIENT]: [%s] Response is longer than packet...", m_remote_hostname.c_str());
    return;
  }
  ...
  if (send(m_fd, buffer.data(), buffer.size(), 0) != (int)buffer.size()) {
    ROS_ERROR("[SERVICE_CLIENT]: [%s] Could not send ack UDP data: %s", m_remote_hostname.c_str(), strerror(errno));
    return;
  }
  ...
  ROS_WARN("[SERVICE_CLIENT]: [%s] Received unexpected UDP service packet response, ignoring", m_remote_hostname.c_str());
}  // namespace nimbro_service_transport
...
int main(int argc, char** argv) {
  ...
  // | ------------------- loading parameters ------------------- |
  mrs_lib::ParamLoader param_loader(nh, "SERVICE_CLIENT");
  ROS_INFO("[SERVICE_CLIENT]: Loading parameters: ");
  ...
  if (robot_names.empty()) {
    ROS_ERROR("[SERVICE_CLIENT]: Robot names list is empty or contains only the hostname of this device!");
    ros::shutdown();
    return -1;
  }

  if (service_list.size() == 0) {
    ROS_ERROR("[SERVICE_CLIENT]: Service list is empty!");
    ros::shutdown();
    return -1;
  }
  ...
  for (const std::string& tmp_name : robot_names) {
    ...
    if (host_entry == NULL) {
      ROS_ERROR("[SERVICE_CLIENT]: Cannot resolve hostname '%s'. It is not specified in '/etc/hosts'.", tmp_name.c_str());
      tmp_valid = false;
      continue;
    }
  ...
  ROS_INFO("[SERVICE_CLIENT]: Resolved 'robot_names':");
  for (auto const& [name, ip] : robot_name_map) {
    ROS_INFO_STREAM("[SERVICE_CLIENT]: " << name << "\t\t ---> \t\t" << ip);
  }
  ...
  for (int32_t i = 0; i < service_list.size(); ++i) {
    ...
    if (service_name.compare(0, 3, "/*/") == 0) {
      for (size_t robot_idx = 0; robot_idx < robot_names.size(); robot_idx++) {
    ...
    // Catch further regex expressions
      ...
      ROS_ERROR("[SERVICE_CLIENT]: Regex expressions [%s] are not allowed, with the exception of regex robot names (i.e. solely '/*/topic_name' is allowed).",
                service_name.c_str());
    }
    // Write down services from specified robots
      ...
      if (!specific) {
        ...
        if (service_name.find("robot") != std::string::npos) {
          ROS_ERROR("[SERVICE_CLIENT]: Given robot of service [%s] is unknown.", service_name.c_str());
        }
        // Catch non-namespace elements without 'robot' tag
        else if (!entry.hasMember("robot")) {
          ROS_ERROR("[SERVICE_CLIENT]: Service [%s] has not unique namespace and its target server is unspecified. Specify its server by adding 'robot' tag.",
                    service_name.c_str());
        } else {
          ...
          if (robot_name_map.count(robot_name) == 0) {
            ROS_ERROR("[SERVICE_CLIENT]: Unknown robot name %s. Ommiting.", robot_name.c_str());
          } else {
            robot_services[robot_name_map[robot_name]][type].push_back(service_name);
          }
        }
  ...
  for (auto const& robot_name : robot_name_map) {
    ROS_INFO("[SERVICE_CLIENT]: Initializing connection to server: %s", robot_name.second.c_str());
    clients.push_back(std::make_unique<nimbro_service_transport::UDPClient>(robot_name.first, robot_name.second, robot_services[robot_name.second],
                                                                            response_timeout, call_timeout, call_repeats, destination_port, topic_prefix));
  }
  ROS_WARN("[SERVICE_CLIENT]: UDP service client initialized");
